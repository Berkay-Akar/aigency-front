...
const FAST_SAMPLE_IMAGES: Record<AiGenerationMode, FastSampleConfig> = {
  ...
  "seedream-model-dressing": {
    ...
    output: "/sample-outputs/seedream4_edit_output.png",
  },
  "seedream-product-scene": {
    type: "combine",
    output: "/sample-outputs/seedream_v45_edit_output.png",
  },
  upscale: null,
};
...
export function PreviewPanel({ className }: { className?: string }) {
  ...
  return (
    <div
      className={cn(
        "flex min-h-0 flex-1 flex-col rounded-3xl border border-border dark:bg-[rgb(10_10_12/0.65)] bg-card/80 shadow-sm backdrop-blur-xl",
        className,
      )}
    >
      <div className="relative flex min-h-[min(70vh,720px)] flex-1 flex-col p-4 md:p-6">
        <AnimatePresence mode="wait">
          <motion.div
            key={isGenerating ? "loading" : result ? "result" : "empty"}
          ...
          >
            <div className="flex w-full max-w-sm lg:max-w-[calc((100vh_-_260px)*0.75)] flex-col items-center gap-4">
              {/* Card — always same aspect ratio */}
              <div className="relative w-full aspect-[3/4] overflow-hidden rounded-2xl border border-white/10 bg-white/3 shadow-[0_8px_40px_rgba(0,0,0,0.35)]">
                ...
                {/* Sample: single image */}
                {showEmpty && fastSampleConfig?.type === "single" && (
                  <>
                ...
                {/* Sample: combine output */}
                {showEmpty && fastSampleConfig?.type === "combine" && (
                  <>
                    <div className="absolute left-3 top-3 z-10 flex items-center gap-1.5 rounded-lg border border-white/12 bg-black/60 px-2.5 py-1 backdrop-blur-md">
                      <ImageIcon
                        className="h-3 w-3 text-indigo-300/80"
                ...
                {/* Sample: video */}
                {showEmpty && fastSampleConfig?.type === "video" && (
                  <>
                    <div className="absolute left-3 top-3 z-10 flex items-center gap-1.5 rounded-lg border border-white/12 bg-black/60 px-2.5 py-1 backdrop-blur-md">
                      <Film
                        className="h-3 w-3 text-indigo-300/80"
              ...
              {/* Below card: helper text when empty */}
              {showEmpty && (
              ...
              {/* Below card: progress bar when generating */}
              {isGenerating && (
              ...
              {/* Below card: action buttons when result ready */}
              {result && !isGenerating && (
